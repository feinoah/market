...
AutoPlayer.prototype = {
	...
	curId : null,
	_defaultInterval : 5,
	_cookiePrefix : "pconline_autoplayer_",
	initialize : function(args) {
		...
		if (self._isDefined(args.showPageId)) {
			self.showPage = self._getElementsById(args.showPageId);
		}
		...
		if (self._isDefined(args.showSourceCompositeId)) {
			self.showSourceComposite = self
					._getElementsById(args.showSourceCompositeId);
		}
		...
		if (null != self.butSpeedGroup) {
			self.butSpeedGroup.find("a[@speed=" + self.interval + "]").trigger(
					"click");
		}
	...
	go : function(index, noturn) {
		...
		if (toUrl.indexOf("#id=" + self.index) == -1) {
			var pos = sUrl = toUrl.indexOf("#");
			if (pos == -1) {
				toUrl += "#id=" + self.index;
			} else {
				toUrl = toUrl.substr(0, pos + 1) + "id=" + self.index;
			}
			location.href = toUrl;
		...
		if (!self._isDefined(item)) {
			self._debug("can't get item data of page " + index);
			if (self.index > 0) {
		...
		if (!noturn) {
			if (self.screen.attr("src") != item.middlePicUrl) {
				...
				/*
				if (self.pvCounter != null
						&& (self.index > 0 || (self.index == 0 && null != self.curId))) {
					var countUrl = "<img src=http://count5.pconline.com.cn/newcount/count.php?channel=703&screen="
							+ screen.width
							+ "*"
							+ screen.height
							+ "&refer="
					...
					document.getElementById('span_count').innerHTML = countUrl;
				}
				*/
			}
		}
		if (null != self.butPrevLink) {
			var prevItem = self.index > 0 ? self.data[self.index - 1]
					: self.data[self.getTotal() - 1];
			var prevHerf = self.baseUrl + prevItem.productId + "_bigpicture"
					+ prevItem.id + (self.tag != "" ? "_tag" + self.tag : "")
					+ ".html";
		...
		if (null != self.butNextLink) {
			var nextItem = self.index >= (self.getTotal() - 1) ? self.data[0]
					: self.data[self.index + 1];
			var nextHerf = self.baseUrl + nextItem.productId + "_bigpicture"
					+ nextItem.id + (self.tag != "" ? "_tag" + self.tag : "")
					+ ".html";
		...
		if(self.butPrev){
			self.butPrev.attr("href", "#id=" + (self.index - 1 > 0 ? self.index - 1 : 0) + "");
		}
		if(self.butNext){
			self.butNext.attr("href", "#id=" + (self.index + 1) + "");
		}
		if(self.butPrevGroup){
			self.butPrevGroup.attr("href", "#id=" + (self.index - 4 > 0 ? self.index - 4 : 0) + "");
			self.butNextGroup.attr("href", "#id=" + (self.index + 5) + "");
		}
		if (null != self.butExamine && self.butExamine.length > 0) {
			...
			/*
			self.butExamine.attr("href",
					"http://www.pconline.com.cn/images/html/viewpic_pconline.htm?"
							+ bigPicUrl + "&channel=703");*/
		}
		if (null != self.showSource) {
			...
			/*if ("网友上传图片" == item.articleTag) {
				...
				source = '<a href="' + item.articleUrl + '" target="_blank">'
						+ item.articleTitle + '</a>';
			}
	...
	setSpeed : function(speed) {
		...
		jQuery.cookie(self._cookiePrefix + "interval", self.interval, {
			"expires" : 999999999,
			"domain" : "android-in-car.com",
			"path" : "/"
		});
		if (self.playFlag) {
	...
	prevGroup : function() {
		...
		self.go(Number(self.index) - Number(self.navSize));
	},
	nextGroup : function() {
		var self = this;
		var goTo = self.index + self.navSize;
		if (goTo >= self.getTotal() - self.navSize) {
			goTo = self.getTotal() - self.navSize
		}
		if (self.index >= goTo) {
			alert("已经是最后一组了");
			return;
	...
	_eventBinding : function() {
		var self = this;
		self._bindingButClick(self.butPlay, function() {
			self.play();
		}, true);
		self._bindingButClick(self.butStop, function() {
			self.stop();
		}, true);
		self._bindingButClick(self.butPause, function() {
			self.pause();
		}, true);
		if (null != self.butSpeedGroup) {
			self.butSpeedGroup.find("a").click(function() {
				self.butSpeedGroup.find("a").removeClass("focus")
				self.setSpeed(Number(jQuery(this).attr("speed")));
				jQuery(this).addClass("focus");
			});
		}
		self._bindingButClick(self.butPrev, function() {
			self.prev();
		}, true);
		self._bindingButClick(self.butNext, function() {
			self.next();
		}, true);
		self._bindingButClick(self.butPrevGroup, function() {
			self.prevGroup();
		}, true);
		self._bindingButClick(self.butNextGroup, function() {
			self.nextGroup();
		}, true);
		...
		self.screen.hover(
				...
				function() {
					...
					jQuery(this).css('cursor', 'auto').attr('title', '').attr(
							'alt', '');
				}).mousemove(
				function(event) {
			...
			var img = self.screen.get(0);
			var height = "";
			var width = "";
			self.screen.css("height", "").css("width", "");
			self._debug("pic:" + img.width + "x" + img.height);
			if (parseInt(img.width) > 500) {
				width = 500;
				height = parseInt((500 / img.width) * img.height);
			} else {
				if (parseInt(img.height) > 500) {
					width = parseInt((500 / img.height) * img.width);
					height = 500;
				}
			}
			self.screen.css("height", height).css("width", width);
			self._debug("style:" + height + "x" + width);
		});
	},
	_navRefresh : function(nocheck) {
		...
		var html = "";
		var begin = Number(self.index) - 1;
		if (begin < 0) {
			begin = 0;
		}
		var end = begin + Number(self.navSize);
		if (end > self.getTotal() - 1) {
			end = self.getTotal();
			begin = end - self.navSize;
			if (begin < 0) {
				begin = 0;
			}
		}
		...
		for ( var i = begin; i < end; i++) {
			...
			html += " <a href=\"#id="
					...
					+ item.thumbPicUrl
					+ "\""
					+ (self._isDefined(item.alt) && "" != item.alt ? "alt=\""
							+ item.alt + "\"" : "") + " /></a>";
			html += "</li>";
		}
		self.nav.html(html);
	...
	_showNumberRefresh : function() {
		...
		if (null != self.showPage) {
			self.showPage.html(self.index + 1);
		}
	},
				...
				.keydown(
						function(event) {
							...
							case 37:
								...
								break;
							case 34:
								var nextItem = self.index < self.data.length - 1 ? self.data[self.index + 1]
										: self.data[0];
								url = self.baseUrl
										+ nextItem.productId
										+ "_bigpicture"
	...
	_filter : function(picFilter) {
		...
		Outer: for ( var i = 0; i < self.data.length; i++) {
			...
			for ( var j = 0; j < tempFilter.length; j++) {
				...
				if (eval("!self._isDefined(item." + kay_val[0] + ") || item."
						+ kay_val[0] + ".indexOf('" + kay_val[1] + "')==-1")) {
					continue Outer;
				}
			}
			data.push(item);
		...
		self
				._debug("filter success," + self.data.length + " to "
						+ data.length);
		self.data = data;
	},
	console : null,
	...
	_debug : function(str) {
		...
		if (null == self.console) {
			if (jQuery("#" + self.consoleId).length <= 0) {
				jQuery("html")
						.append("<div id='" + self.consoleId + "'></div>");
			}
			self.console = jQuery("#" + self.consoleId);
		}
		self.console.append("[" + new Date() + "]:" + str + "<br />");
	},
	_destory : function() {
	}
}