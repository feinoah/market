...
},message:"Please enter a value between {0} and {1}."},number:{validator:function(v){return /^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(v);},message:'只能输入数字.'},
notempty:{validator:function(v){return $.trim(v).length>0;},message:'值不能为空.'},
mobile:{validator:function(v){return /^(((15[012356789]{1})|(18[02356789]{1})|(13[0-9]{1})|(14[57]{1}))+\d{8})$/.test(v);},message:'手机号码不正确'},
safepass:{validator:function(v){return !(/^(([A-Z]*|[a-z]*|\d*|[-_\~!@#\$%\^&\*\.\(\)\[\]\{\}<>\?\\\/\'\"]*)|.{0,5})$|\s/.test(v));},message:'密码必须是六位以上的数字字母组合'},
